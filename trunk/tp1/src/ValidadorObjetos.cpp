...
bool ValidadorObjetos::ValidarBasicos(int x, int y, std::string color, int rotacion, int masa, bool estatico) {

	std::regex e ("/^#[a-fA-F0-9]{6}$/");

	if (0<x<100 ^ 0<y<100 ^ std::regex_match(color,e) ^ (rotacion>=0) ^ (masa>0 || (masa==0 ^ estatico ==true))){
	return true;
	}
	return false;
}
...
bool ValidadorObjetos::ValidarParalelogramoNoRectangulo(int base, int ladoaltura, int alfa){
