...
//		Log::Loguear(mensajeError, nombreArchivo);
		...
		*ancho_un = un_Escenario.get("ancho_un", 50).asInt();
		...
		int* personaje_x = new int;
		int* personaje_y = new int;
		...
		*personaje_y = un_Escenario["personaje"].get("y", -100).asInt();
		...
		for (size_t i = 0; i < objetos.size(); i++) {
			std::string* tipo = new std::string();
			*tipo = objetos[i].get("tipo", "rectangulo").asString();
			int* x = new int;
			int* y = new int;
			int* diametro = new int;
			int* ancho = new int;
			int* alto = new int;
			int* base = new int;
			int* altura = new int;
			...
			int* beta = new int;
			int* lados = new int;
			int* rotacion = new int;
			int* masa = new int;
			bool* estatico = new bool;
			float* escala = new float;
			...
			*alto = objetos[i].get("alto", 10).asInt();

			*base = objetos[i].get("base", 10).asInt();
			*altura = objetos[i].get("altura", 10).asInt();
			*alfa = objetos[i].get("alfa", 25).asInt();
			*beta = objetos[i].get("beta", 25).asInt();

			*lados = objetos[i].get("lados", 0).asInt();

			std::string* color = new std::string();
			*color = objetos[i].get("color", "#FF0000").asString();

			*rotacion = objetos[i].get("rot", 0).asInt();
			*masa = objetos[i].get("masa", 10).asInt();
			*estatico = objetos[i].get("estatico", false).asBool();
			*escala = objetos[i].get("escala", 15).asFloat();
