...
void Pantalla::inicializar() throw (SDL_Excepcion){
	...
	ventana = SDL_CreateWindow("SnowBross", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, anchoPx, altoPx, SDL_WINDOW_RESIZABLE);
	if (ventana == nullptr){
	...
	SDL_Surface *direcPNG = NULL;
	direcPNG = IMG_Load(dirImg);
	this->fondo = SDL_CreateTextureFromSurface(renderer, direcPNG);
	SDL_FreeSurface(direcPNG);
